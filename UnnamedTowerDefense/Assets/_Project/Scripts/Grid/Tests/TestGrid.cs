...
{
    [AddComponentMenu("Tests/Grid/Test Grid")]
    public class TestGrid : Grid<TestCell>
    {
        ...
        private void Initialize()
        {
            ...
            for (int i = 0; i < width; i++)
            {
                ...
                {
                    Cells[i, j] = new TestCell(new Vector2Int(i, j), horizontalSpace, verticalSpace, this);
                    Cells[i, j].OnClicked += OnClick;
                }
            }
        }

        private void OnClick(TestCell clicked)
        {
